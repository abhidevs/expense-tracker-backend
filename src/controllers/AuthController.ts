...
import { comparePassword } from "../utils/encryptPass";
...
export const registerUser = async (req: Request, res: Response) => {
  try {
    const checkUser: any = await UserService.checkIfUserExists(req.body);
    if (checkUser) {
...
export const getResetPassword = async (req: Request, res: Response) => {
  try {
    const check: any = await AuthService.checkForgetPassRequest(req.params.id);
    if (!check) {
